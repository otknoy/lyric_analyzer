...
class Term:
    ...
    def is_ascii(self):
        r = re.compile(r'\A[\x00-\x7f]*\Z')
        return r.match(self.basic_form)
...
def remove_stopword(terms, stopwords=STOPWORDS):
    ...
    terms = [t for t in terms if not t.is_ascii()]
...
if __name__ == '__main__':
    ...
    print " normalized"
    tfidf_list = tf_idf(docs, normalize=True)
    for tfidf in tfidf_list:
        print " | ".join(["%s, %0.2f" % (t, f) for t, f in tfidf.items()])
